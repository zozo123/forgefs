...
| I19 | A checkin publishes every overlay entry the session holds, or refuses and names what it would leave behind. Checkin folds exactly one mount, so a session holding staged entries under any other mount is refused with exit 1 and those mounts named; a checkin that returns -- `updated`, `forked` or `noop` -- therefore always means the namespace holds no staged work. I18 forbids the failure path that destroys staged work; I19 forbids the success path that hides it, so `checkin` and `abandon` can never disagree about whether a session still holds work. |
...
| I19 | `forge-api/workspace.rs`, `forge-store/meta.rs`, `forge-cli/main.rs` | `checkin_staged_work.rs`, `cli_checkin_staged_work.rs`, `gc_and_abandon.rs`, `CLI_ABI.md` |
