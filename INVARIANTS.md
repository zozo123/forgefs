...
| I5 | Refs move only expected→new. Lost CAS forks or denies. Protected refs deny. A lost SESSION checkin forks to `heads/agents/<agent>/forks/<ref>/<ulid>` -- inside the losing agent's own scope, because I18 retargets that session's mount at the fork and a ref the session cannot open is not a place work can be preserved to (#343). `merge` and `import` retarget no session and still fork to `forks/<ref>/<agent>/<ulid>`. |
