...
- **A read-write mount at a non-root path does not show the ref it names.** `mount /w --rw ref:heads/topic` resolves `heads/topic` once, to validate it, and then serves and folds the session's own single pinned commit instead: `ls /w` and `read /w/...` show the session's base, not `heads/topic`, and `checkin /w` folds onto that base and CASes `heads/topic` from it. Treat a non-root `--rw` mount as unsupported until this is fixed.
- **A successful checkin repins every mount, not the one it published.** A session has one `pinned_oid`; checking in mount A moves it, so what mount B reads changes underneath it, and an observation recorded through B is then re-validated against B's *live* ref rather than the base it was read from. Once that ref has moved, no checkin on any mount can succeed, `abandon` without `--discard` refuses over staged work, and the session is stuck.
- **A session holding staged work can be stranded.** `main` is protected, so a `--rw` mount on it accepts writes that `checkin` then refuses (`ref main is protected`), and `abandon` refuses too because work is staged. A `--rw oid:` mount is the same shape (`cannot checkin an oid mount`). The only exits are `abandon --discard`, which destroys the work, or re-mounting the path onto a writable ref, which silently changes where the work lands. There is no invariant stating this liveness property yet; `crates/forge-api/tests/model_composition.rs` states it and reproduces the failure.
