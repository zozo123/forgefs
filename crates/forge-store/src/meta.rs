...
/// produces it from inside the write transaction that unlinks (I19).
///
...
pub struct GcCatalogRoots {
    ...
    pub live_refs: usize,
    /// `(namespace, path, spec)` for every mount.
...
fn read_gc_roots(conn: &Connection) -> Result<GcCatalogRoots> {
    ...
    {
        let mut stmt = conn
            .prepare("SELECT ns_id, path, spec FROM mounts ORDER BY ns_id, path")
            .map_err(map_sql)?;
        let rows = stmt
            .query_map([], |r| {
                Ok((
            ...
            .map_err(map_sql)?;
        for row in rows {
            out.mounts.push(row.map_err(map_sql)?);
        }
    }
    {
        let mut stmt = conn
...
impl GcSweepTxn<'_> {
    ...
    /// sweep's transaction makes the two facts commit together (I6, I19).
