...
one commit per agent onto its own ref, 5 fresh-repository repetitions per
configuration. Medians are run-level medians (median_low).

| Configuration | ops/s | p50 ms | p95 ms | p99 ms | max ms |
|---|---|---|---|---|---|
| ForgeFS (synchronous=FULL, file+dir fsync) | 462.9 | 8.39 | 10.17 | 12.86 | 12.86 |
| ForgeFS through the `forge` CLI, same durability (3 execs/agent) | 198.3 | 19.00 | 28.51 | 39.71 | 39.71 |
| Git worktrees, as-shipped default | 289.2 | 14.11 | 19.51 | 22.24 | 22.24 |
| Git worktrees, core.fsync=all fsyncMethod=fsync | 283.3 | 12.65 | 20.44 | 25.14 | 25.14 |
...
Process-spawn floor (same driver, `git rev-parse --verify HEAD` per agent): 1639.2 ops/s, p50 2.20 ms. Each Git agent operation costs two more execs than that; ForgeFS W1 runs in-process threads. Subtract this before attributing any part of the gap to storage design.

Process-model note. `forge bench` drives its agents as in-process threads while Git execs twice per agent. The `forge` CLI row above runs the same ForgeFS work with three execs per agent and is the row to compare against Git if your agents shell out. At 4 workers the no-op Git exec floor alone is 1639.2 ops/s, so any Git deficit smaller than that floor is process-model cost and not storage design.

Excluded from every timed region on both sides: repository creation, and per agent worktree/branch creation (Git, median 0.807 s for all agents) versus capability grant and session open (ForgeFS, inside the W1 timed region).
...
storage lifetime puts=241 bytes=unavailable fsync_file=241 fsync_file_us=51794 fsync_dir=640 fsync_dir_us=63507 lifetime_barrier_us=115301
sqlite lifetime  lock_acquires=688 lock_wait_us=30393 txn_count=161 txn_us=68151 lifetime_accounted_us=98544 busy=0 updated=80 forked=0 denied=0
```
